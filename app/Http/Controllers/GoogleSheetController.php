...
class GoogleSheetController extends Controller
{
    ...
    public function seniortrafollow(Request $request)
    {
        ...
        // ... (keep all existing query code exactly the same)
        ...
        // ✅ FIX: Return partial view for AJAX requests
        if ($request->ajax()) {
    ...
    public function seniorpaidins(Request $request)
    {
        ...
        $query->where('Exe_Remarks', 'Ready To Pay')->where('installment', 1);
        ...
        // -----------------------------
        if ($request->ajax()) {
            return view('database.partials.seniorfollow_table', [
                'data' => $pagedData,
                'juniorUsers' => $juniorUsers
            ])->render();
        }

        return view('database.seniorfollow', [
            'data' => $pagedData,
            'juniorUsers' => $juniorUsers
        ]);
    }
    ...
    public function seniorcon(Request $request)
    {
        ...
        // ✅ REMOVE PHP FILTERING - use DB result directly
        $results = $query->orderBy('updated_at', 'desc')->get();
        ...
        return view('database.seniorcon', [
            'data' => $pagedData,
            'juniorUsers' => $juniorUsers
        ]);
    }
    ...
    public function seniortraSuggestions(Request $request)
    {
        ...
        if ($query && strlen($query) >= 3) {
            // ✅ USE SAME BASE QUERY AS seniortra() METHOD
            $userPattern = "%:" . $authUser->id . "|junior";
            $zeroPattern = "%:0|senior";

            $queryBuilder = GoogleSheetData::where(function ($main) use ($authUser, $userPattern, $zeroPattern) {
                ...
                // ✅ ADD TRANSFER STATUS FILTERS (same as seniortra)
                ->where(function ($q) {
                    $q->whereNull('TransferRemark')
                        ->orWhere('TransferRemark', '');
                })
            ...
            // ✅ APPLY JUNIOR GROUP FILTER (same as seniortra)
            $assignedJuniorIds = $authUser->group ?? [];
    ...
    public function seniortraotpSuggestions(Request $request)
    {
        ...
        if ($query && strlen($query) >= 3) {
            // ✅ USE SAME BASE QUERY AS seniortraotp() METHOD
            $userPattern = "%:" . $authUser->id . "|junior";
            $zeroPattern = "%:0|senior";
            ...
            $queryBuilder = GoogleSheetData::where(function ($main) use ($authUser, $userPattern, $zeroPattern) {
                ...
                // ✅ ADD TRANSFER STATUS FILTERS (same as seniortraotp)
                ->where(function ($q) {
                    $q->whereNull('TransferRemark')
                        ->orWhere('TransferRemark', '');
                })
            ...
            // ✅ APPLY JUNIOR GROUP FILTER (same as seniortraotp)
            $assignedJuniorIds = $authUser->group ?? [];
            $results = $results->filter(function ($item) use ($assignedJuniorIds, $authUser) {
                if (empty($item->created_by)) return false;
    ...
    public function juniorvm(Request $request)
    {
        $authUser = Auth::user();
        $search = $request->input('search');
        $rowId = $request->input('row_id');
