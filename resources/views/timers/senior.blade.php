...
                                <button id="resumebreak_{{ $timer['user_id'] }}" class="senior-btn resumebreak_{{ $timer['user_id'] }}" data-type="resumebreak" data-user="{{ $timer['user_id'] }}" style="width:65px;height:28px;border-radius:14px;background:#d4edda;border:1px solid #28a745;display:flex;align-items:center;justify-content:center;font-size:12px;color:#28a745; {{ ['lunch', 'tea', 'break']->contains($timer['pause_type']) ? 'display:flex;' : 'display:none;' }}">
                                    <iconify-icon icon="mdi:play" style="margin-right:2px;font-size:14px;"></iconify-icon>Resume
                                </button>
                                <button id="lunch_{{ $timer['user_id'] }}" class="senior-btn lunch_{{ $timer['user_id'] }}" data-type="lunch" data-user="{{ $timer['user_id'] }}" style="width:65px;height:28px;border-radius:14px;background:#f8f9fa;border:1px solid #ddd;display:flex;align-items:center;justify-content:center;font-size:12px;color:#ffc107; {{ ['lunch', 'tea', 'break']->contains($timer['pause_type']) ? 'display:none;' : 'display:flex;' }}">
                                    <iconify-icon icon="mdi:food" style="margin-right:2px;font-size:14px;"></iconify-icon>Lunch
                                </button>
                                <button id="tea_{{ $timer['user_id'] }}" class="senior-btn tea_{{ $timer['user_id'] }}" data-type="tea" data-user="{{ $timer['user_id'] }}" style="width:65px;height:28px;border-radius:14px;background:#f8f9fa;border:1px solid #ddd;display:flex;align-items:center;justify-content:center;font-size:12px;color:#8b4513; {{ ['lunch', 'tea', 'break']->contains($timer['pause_type']) ? 'display:none;' : 'display:flex;' }}">
                                    <iconify-icon icon="mdi:coffee" style="margin-right:2px;font-size:14px;"></iconify-icon>Tea
                                </button>
                                <button id="break_{{ $timer['user_id'] }}" class="senior-btn break_{{ $timer['user_id'] }}" data-type="break" data-user="{{ $timer['user_id'] }}" style="width:65px;height:28px;border-radius:14px;background:#f8f9fa;border:1px solid #ddd;display:flex;align-items:center;justify-content:center;font-size:12px;color:#007bff; {{ ['lunch', 'tea', 'break']->contains($timer['pause_type']) ? 'display:none;' : 'display:flex;' }}">
...
                            <span class="pause-badge pause-badge_{{ $timer['user_id'] }} px-3 py-2 radius-8 shadow-sm {{ ['lunch', 'tea', 'break']->contains($timer['pause_type']) ? 'bg-danger text-white' : 'bg-primary text-white' }}" data-user="{{ $timer['user_id'] }}">
...
                        // alert(`User ${data.user_id} has finished their 9-hour session.`); // Commented out to prevent excessive alerts
                    }
                });
            })
...
        // Function to toggle the pause/resume buttons visibility for a specific user
        function togglePauseButtons(userId, newPauseType) {
            const container = document.getElementById(`seniorcontrolButtons_${userId}_container`);
            if (!container) return;

            const resumeBtn = document.getElementById(`resumebreak_${userId}`);
            const pauseBtns = [
                document.getElementById(`lunch_${userId}`),
                document.getElementById(`tea_${userId}`),
                document.getElementById(`break_${userId}`)
            ].filter(Boolean);

            const isPaused = ['lunch', 'tea', 'break'].includes(newPauseType);
            
            if (isPaused) {
                pauseBtns.forEach(btn => btn.style.display = 'none');
                if (resumeBtn) resumeBtn.style.display = 'flex';
            } else { // Covers 'resume' and any other non-pause status
                pauseBtns.forEach(btn => btn.style.display = 'flex');
                if (resumeBtn) resumeBtn.style.display = 'none';
            }

            // Update Badge (optional, but good to keep in one place)
            const badge = document.querySelector(`.pause-badge_${userId}`);
            if (badge) {
                const type = newPauseType || 'resume';
...
                // Ensure listener isn't attached multiple times due to polling
...
                        .then(res => res.json())
                        .then(data => {
                            // Update Timer Widget details (already existed)
...
                            }
                            ...
                            // Immediately update buttons based on the new pause_type from the server response
                            // This is the key fix for immediate user experience.
                            togglePauseButtons(userId, data.pause_type);
                        })
...
        });
        
        // Polling function to catch updates from other sources (e.g., another senior's action)
...
                .then(res => res.json())
                .then(data => {
                    // Call the unified toggle function
                    togglePauseButtons(userId, data.pause_type);
                })
...
        pollPauseButtons(); // Initial call
        setInterval(pollPauseButtons, 1000); // Polling every second

    });
</script>

<script>
    // Login/Logout Toggle logic (kept separate for clarity)
    document.addEventListener("DOMContentLoaded", function() {
        function updateButton(button, status, userId) {
            const cardContainer = button.closest('.user-grid-card');
            // The buttons themselves are dynamically generated in PHP, so we update the parent structure
            // For simplicity in JavaScript, we'll try to update the innerHTML/style directly
            // WARNING: Directly replacing innerHTML can cause listeners to be lost.
            // A more robust solution would re-fetch the entire card on status change or rely on full page reload/re-render.
            // Since the original code relies on direct element manipulation, we'll mimic that.
            const loginBtn = document.querySelector(`#loginBtn_${userId}`);
            const logoutBtn = document.querySelector(`#logoutBtn_${userId}`);

            // This function is problematic because it tries to update a button that might not exist 
            // after a state change. A more common solution is to replace the button entirely
            // or abstract this into a single button whose text/style changes.
            // Since the initial PHP renders both a login OR a logout button based on $timer['button_status'],
            // we'll assume the back-end handles the full re-render of the card or we manage the DOM switch.

            if (status == 0) {
                // Junior is disabled. Should show "Enable Junior" button (loginBtn)
                if (logoutBtn) {
                ...
                // Junior is enabled. Should show "Disable Junior" button (logoutBtn)
                if (loginBtn) {
...
                .then(data => {
                    if (data.success) {
                        // The server *should* return the new button_status here
                        // We use the same update logic as the poller for immediate feedback
...
        // Use a delegation approach for all 'senior-btn' clicks (covers both enable/disable and pause/resume)
        document.body.addEventListener("click", function(e) {
            const btn = e.target.closest(".senior-btn");
            if (btn && (btn.dataset.type === 'login' || btn.dataset.type === 'logout')) {
...
                        data.forEach(user => {
                            // We can use the login/logout buttons as a generic target since they are uniquely identified
                            // and then call the update function to toggle their visibility.
