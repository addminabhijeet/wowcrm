...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Function to handle senior control button clicks for ALL users
        function setupSeniorControlButtons() {
            document.querySelectorAll('.senior-btn').forEach(btn => {
                // Remove existing listeners to prevent duplicates
                btn.removeEventListener('click', handleSeniorButtonClick);
                btn.addEventListener('click', handleSeniorButtonClick);
            });
        }

        function handleSeniorButtonClick(event) {
            ...
            fetch("{{ route('button.status') }}")
                ...
                .then(data => {
                    if (Array.isArray(data)) {
                        data.forEach(user => {
                            const button = document.querySelector(`button[data-user='${user.user_id}'][data-type='login'], button[data-user='${user.user_id}'][data-type='logout']`);
