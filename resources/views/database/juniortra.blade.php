                        ...
                        <tbody id="sheet-table-body">
                            @foreach ($data as $row)
                                <tr id="row-{{ $row->id }}" data-id="{{ $row->id }}">
                                    ...
                                    {{-- Remark --}}
                                    <td>
                                        <textarea class="form-control remark-autocomplete" data-key="Remark" placeholder="Type remark" rows="2"
                                            style="width: 200%; height: calc(2 * 2em);">{{ $row->Remark ?? '' }}</textarea>
                                    </td>
    ...
    <script>
        $(document).ready(function() {
            ...
            function fetchTable(search = '', page = 1, junior_user = '', row_id = '') {
                $.ajax({
                    url: "{{ route('google.sheet.junior.tra') }}",
                    type: 'GET',
                    data: {
                        search,
                        page,
