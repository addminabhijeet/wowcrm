        ...
        <table class="table bordered-table sm-table mb-0">
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    function syncTextarea(td) {
                        const remark = td.querySelector('textarea.remark-autocomplete');
                        const transfer = td.querySelector('textarea.transferremark-autocomplete');

                        if (remark) {
                            td.querySelector('input[name="Remark"]').value = remark.value.trim();
                        }

                        if (transfer) {
                            td.querySelector('input[name="TransferRemark"]').value = transfer.value.trim();
                        }
                    }
                    ...
                    document.addEventListener('input', function(e) {
                        if (
                            e.target.classList.contains('remark-autocomplete') ||
                            e.target.classList.contains('transferremark-autocomplete')
                        ) {
                            syncTextarea(e.target.closest('td'));
                        }
                    });

                    // 🛡️ Final sync before any submit
                    form.addEventListener('submit', function() {
                        document.querySelectorAll('#sheet-table-body tr').forEach(tr => {
                            tr.querySelectorAll('td').forEach(td => syncTextarea(td));
                        });
                    });
                });
                ...
                $(document).on('click', '.save-btn', function() {
                    ...
                    $tr.find('td').each(function() {
                        ...
                        let remark = $td.find('textarea.remark-autocomplete');
                        let transfer = $td.find('textarea.transferremark-autocomplete');

                        if (remark.length) {
                            $td.find('input[name="Remark"]').val(remark.val().trim());
                        }

                        if (transfer.length) {
                            $td.find('input[name="TransferRemark"]').val(transfer.val().trim());
                        }
                    });
