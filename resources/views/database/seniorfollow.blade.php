...
// If table loads via AJAX / tab switch
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function initDatePickers(context = document) {
            const laravelToday = "{{ \Carbon\Carbon::now('America/New_York')->format('m/d/Y') }}";

            const inputs = context.querySelectorAll('input.date-picker');

            inputs.forEach(input => {

                // Avoid double init (important fix for tabs/AJAX)
                if (input.classList.contains('flatpickr-input-initialized')) return;
                input.classList.add('flatpickr-input-initialized');
                ...
                const opts = {
                    dateFormat: "m/d/Y",
                    allowInput: true,
                    defaultDate: input.value || null, // 🔥 KEY FIX (prevents blank on tab)
                    onChange: function(selectedDates, dateStr) {
                        input.style.backgroundColor = dateStr ? dateColor : '#fff';
                    },
                    onReady: function() {
                        if (input.value) {
                            input.style.backgroundColor = dateColor;
                        }
                    }
                };

                if (key === "Date") {
                    opts.maxDate = laravelToday;
