            ...
            <table class="table bordered-table sm-table mb-0">
                ...
                <tbody id="sheet-table-body">
                    ...
                    <tr id="row-{{ $row->id }}" data-id="{{ $row->id }}">
                        ...
                        {{-- Exe Remarks --}}
                        <td>
                            @php $exeOptions = ['Training Completed','Not Interested','Not Connected','Did Not Pickup','Others','Payment Completed','VM','Busy']; @endphp
        ...
        function addBlankRow() {
            ...
            colKeys.forEach(k => {
                if (['Exe Remarks', 'Immigration', 'Relocation', '1st Follow Up Remarks', 'Course', 'Time Zone', 'Qualification'].includes(k)) {
                    ...
                    if (k === 'Immigration') opts = ['F1 CPT', 'F1 OPT', 'STEM OPT', 'HIB', 'B2', 'B1', 'H4', 'H4 EAD', 'GC EAD','GC/PR', 'USC'];
...
<script>
    $(document).ready(function() {
        ...
        function fetchTable(search = '', page = 1, junior_user = '', row_id = '') {
            $.ajax({
                url: "{{ route('google.sheet.trainer') }}",
                type: 'GET',
                data: {
                    search,
                    page,
...
<script>
    document.getElementById('junior-filter').addEventListener('change', function() {
        ...
        fetch("{{ route('google.sheet.trainer') }}?junior_user=" + juniorId + "&search=" + search, {
                headers: {
