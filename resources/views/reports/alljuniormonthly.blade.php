            ...
            <div class="card-body p-24">
                ...
                <div class="d-flex align-items-center gap-1 justify-content-between mb-4">
                    <div>
                        ...
                        <h5 class="fw-semibold mb-0" id="semiGaugeLabel">{{ $totalCallsToday }}</h5>
                    </div>
                ...
                <div class="d-flex align-items-center gap-1 justify-content-between mb-4">
                    <div>
                        ...
                        <h5 class="fw-semibold mb-0" id="areaChartLabel">{{ array_sum($callsPerMonth) }}</h5>
                    </div>
                ...
                <div class="d-flex align-items-center gap-1 justify-content-between">
                    <div>
                        ...
                        <h5 class="fw-semibold mb-0" id="miniBarLabel">{{ array_sum($daysThisWeek) }}</h5>
                    </div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const chartData = {
        yearly: {
            main: JSON.parse('@json($callsPerMonth)'),
            mainCategories: ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],
            area: JSON.parse('@json($callsPerMonth)'),
            mini: JSON.parse('@json($daysThisWeek)'),
            label: "Yearly overview",
            semi: $totalCallsToday 
        },
        monthly: {
            main: JSON.parse('@json($callsPerHour)'),
            mainCategories: JSON.parse('@json($hourLabels)'),
            area: JSON.parse('@json($callsPerHour)'),
            mini: JSON.parse('@json($daysThisWeek)'),
            label: "Monthly overview",
            semi: $totalCallsToday 
        },
        weekly: {
            main: JSON.parse('@json($daysThisWeek)'),
            mainCategories: JSON.parse('@json($dayLabels)'),
            area: JSON.parse('@json($daysThisWeek)'),
            mini: JSON.parse('@json($daysThisWeek)'),
