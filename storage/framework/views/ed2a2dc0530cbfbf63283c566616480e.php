                    ...
                    <tr id="row-<?php echo e($row->id); ?>" data-id="<?php echo e($row->id); ?>">
                        ...
                        <td>
                            <div class="position-relative">
                                <input type="text"
                                    class="form-control date-picker ps-5 shadow-sm border-0 rounded-3"
                                    style="background-color:#f8f9fa; height:40px;"
                                    data-key="Date"
                        ...
                        <td>
                            <textarea type="text" name="Remark_hidden" class="form-control remark-autocomplete" placeholder="Type remark"
                                rows="6" readonly><?php echo e($row->Remark ?? ''); ?></textarea>

                            <input type="hidden" name="Remark"
                                class="form-control remark-autocomplete remark-hidden" data-key="Remark"
                                value="<?php echo e($row->Remark ?? ''); ?>" placeholder="Type remark" readonly>
                        </td>
...
<script>
    $(document).ready(function() {
        ...
        function fetchTable(search = '', page = 1, junior_user = '', row_id = '', date = '') {
            $.ajax({
                url: "<?php echo e(route('google.sheet.seniortraotp')); ?>",
                type: 'GET',
                data: {
                    search,
                    page,
        ...
        const showSuggestions = debounce(function() {
            ...
            $.ajax({
                url: "<?php echo e(route('seniortraotp.suggestions')); ?>",
                type: 'GET',
                data: {
                    query: query,
...
<script>
    $(document).on("click", ".rejected-btn", function() {
        ...
        $.ajax({
            url: "<?php echo e(route('junior.rejected.update')); ?>",
            method: "POST",
            data: {
                id: id,
...
<?php echo $__env->make('layout.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/norloxsolutionscrm.com/wowcrm/resources/views/database/seniortraotp.blade.php ENDPATH**/ ?>