                        ...
                        <td class="text-center align-middle">
                         ...
                         <td>
                             ...
                             <button type="button" class="btn btn-sm btn-warning upload-audio-btn">
                                 <?php echo e(!empty($row->audio) ? 'Change Audio' : 'Upload Audio'); ?>

                             </button>

                             <?php if(!empty($row->audio)): ?>
                             <a href="<?php echo e(url('dashboard/senior/google-sheet/view-audio/' . $row->id)); ?>"
                                 target="_blank" class="btn btn-sm btn-primary play-audio-btn">
                                 Play Audio
                             </a>

                             <a href="<?php echo e(url('dashboard/senior/google-sheet/download-audio/' . $row->id)); ?>"
                         ...
                         <td>
                             <textarea type="text" name="TransferRemark_hidden" class="form-control transferremark-autocomplete data-field"
                                 data-key="TransferRemark" placeholder="Type remark" rows="6"><?php echo e($row->TransferRemark ?? ''); ?></textarea>
                         ...
                         <td>
                             <input type="checkbox"
                                 ...
                                 value="0"
                                 <?php echo e((int)$row->installment === 1 ? 'checked' : ''); ?>>

                             <input type="hidden"
                                 name="installment"
                                 class="installment-hidden"
                                 data-key="installment"
                                 value="<?php echo e((int)($row->installment ?? 0)); ?>">
                         </td>
                         ...
                         <td>
                             <?php $exeOptions = ['Verification Completed','Payment Completed']; ?>
     ...
     document.addEventListener("DOMContentLoaded", function() {
         ...
         tableBody.addEventListener('click', function(e) {
             if (e.target.matches('.save-btn') || e.target.closest('.save-btn')) {
                 ...
                 let url, method;
                 if (id === "new") {
                     url = "<?php echo e(route('seniorstoremod')); ?>";
                     method = "POST";
                 } else {
                     url = "<?php echo e(route('seniorupdatecon')); ?>";
                     method = "POST";
 ...
 <script>
     $(document).ready(function() {
         ...
         function fetchTable(search = '', page = 1, junior_user = '', row_id = '') {
             $.ajax({
                 url: "<?php echo e(route('google.sheet.seniormod')); ?>",
                 type: 'GET',
                 data: {
                     search,
                     page,
         ...
         const showSuggestions = debounce(function() {
             ...
             $.ajax({
                 url: "<?php echo e(route('senior.suggestionspaid')); ?>",
                 type: 'GET',
                 data: {
                     query
                 },
 ...
 <script>
     document.getElementById('junior-filter').addEventListener('change', function() {
         ...
         fetch("<?php echo e(route('google.sheet.seniormod')); ?>?junior_user=" + juniorId + "&search=" + search, {
                 headers: {
...
<?php echo $__env->make('layout.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/norloxsolutionscrm.com/wowcrm/resources/views/database/seniorcon.blade.php ENDPATH**/ ?>