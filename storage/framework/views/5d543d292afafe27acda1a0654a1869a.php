                ...
                <?php $__currentLoopData = $timers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $timer): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="col-xxl-3 col-md-6 user-grid-card">
                        <div class="position-relative border radius-16 overflow-hidden" style="padding-top: 30px;">
                            <img src="<?php echo e($timer['image']
                            ...
                            <div class="ps-16 pb-16 pe-16 text-center mt--50">
                                <img src="<?php echo e($timer['image'] ? asset('storage/app/public/' . $timer['image']) : asset('assets/images/user-grid/user-grid-bg1.png')); ?>"
                                ...
                                <!-- Timer Widget -->
                                <div class="timer-widget" data-user="<?php echo e($timer['user_id']); ?>"
                                    data-remaining="<?php echo e($timer['remaining_seconds']); ?>"
                                    data-elapsed="<?php echo e($timer['elapsed_seconds']); ?>" data-status="<?php echo e($timer['status']); ?>"
                                    ...
                                    <!-- Control Buttons -->
                                    <div class="seniorcontrolButtons" id="seniorcontrolButtons_<?php echo e($timer['user_id']); ?>"
                                        style="display:flex;align-items:center;gap:4px;flex-wrap:wrap;">
                                        <button data-type="resumebreak" data-user="<?php echo e($timer['user_id']); ?>"
                                        ...
                                        <button data-type="lunch" data-user="<?php echo e($timer['user_id']); ?>"
                                        ...
                                        <button data-type="tea" data-user="<?php echo e($timer['user_id']); ?>"
                                        ...
                                        <button data-type="break" data-user="<?php echo e($timer['user_id']); ?>"
                                ...
                                <?php if($timer['button_status'] == 0): ?>
                                    <button data-type="login" data-user="<?php echo e($timer['user_id']); ?>"
                                ...
                                <?php else: ?>
                                    <button data-type="logout" data-user="<?php echo e($timer['user_id']); ?>"
                                ...
                                <!-- Login/Logout Button -->
                                <?php if($timer['status'] == 0): ?>
                                    <!-- Login Button -->
                                    <button class="user-action-btn" data-type="login"
                                        data-user-id="<?php echo e($timer['user_id']); ?>"
                                        style="width:100%; height:40px; border-radius:14px; background:#0d6efd; border:1px solid #b0c6e6ff; color:#fff; display:flex; align-items:center; justify-content:center; font-size:14px; gap:6px; margin-top:16px; cursor:pointer;">
                                        <iconify-icon icon="mdi:play" style="font-size:16px;"></iconify-icon>
                                        Log In
                                    </button>
                                ...
                                <div class="position-absolute top-0 end-0 me-16 mt-16"
                                    id="badgeContainer_<?php echo e($timer['user_id']); ?>">
                                    <?php if(!empty($timer['pause_type'])): ?>
                                        <?php if($timer['pause_type'] == 'lunch'): ?>
                                            <span class="badge bg-danger text-white px-3 py-2 radius-8 shadow-sm"
                                                ...
                                                Break</span>
                                        <?php elseif($timer['pause_type'] == 'tea'): ?>
                                            <span class="badge bg-success text-white px-3 py-2 radius-8 shadow-sm"
                                                ...
                                                Break</span>
                                        <?php elseif($timer['pause_type'] == 'break'): ?>
                                            <span class="badge bg-warning text-white px-3 py-2 radius-8 shadow-sm"
                                                ...
                                                Break</span>
                                        <?php elseif($timer['pause_type'] == 'resume'): ?>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.user-action-btn').on('click', function() {
                ...
                var url = actionType === 'logout' ? "<?php echo e(route('ajax.logout')); ?>" :
                    "<?php echo e(route('ajax.login')); ?>";
            ...
            function checkButtonStatus() {
                $.ajax({
                    url: "<?php echo e(route('ajax.logincheckStatus')); ?>",
                    method: 'GET',
                    success: function(res) {
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function updatePauseBadges() {
                fetch("<?php echo e(route('timers.latestPauseTypes')); ?>")
                    .then(response => response.json())
                    .then(data => {
                        data.forEach(item => {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updatePauseButtonsPerUser() {
                document.querySelectorAll('.seniorcontrolButtons').forEach(container => {
                    ...
                    fetch('<?php echo e(route('timer.checkPauseButtonsSenior')); ?>', {
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                                'Content-Type': 'application/json'
...
<?php echo $__env->make('layout.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/norloxsolutionscrm.com/wowcrm/resources/views/timers/senior.blade.php ENDPATH**/ ?>